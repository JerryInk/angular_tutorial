...
## Шаг 3: Модальное окно подтверждения удаления через TuiResponsiveDialogService
...
Импортируем ```TuiResponsiveDialogService``` в класс компонента:
```typescript
  import { TuiResponsiveDialogService } from '@taiga-ui/core';// ...protected readonly dialogs = inject(TuiResponsiveDialogService);
...
```typescript
  confirmDelete(id: number | undefined): void {
    ...
    const data: TuiConfirmData = {
      content:
          'Вы уверены, что хотите безвозвратно удалить данную запись?',
      yes: 'Подтвердить',
      no: 'Отмена',
    };
        ...
        next: (response) => {
          if (response) {
            ...
            this.apiService.deleteCourse(id).subscribe(() => {
              ...
              this.notifications.open('Объект успешно удален из базы данных.').subscribe();
            });
          }
        }
      });
...
## Шаг 4: Тестирование сквозного UI-сценария
   ...
   3. При нажатии кнопки подтверждения окно исчезает, отправляется запрос к бэкенду, список OnPush перерисовывается, и в правом верхнем углу экрана всплывает тост-уведомление.
