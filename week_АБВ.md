...
```typescript
  ...
  //  ПРАВИЛЬНО ДЛЯ v5: Импортируем строго Standalone компоненты и директивы
  import { TuiInput, TuiInputNumber, TuiTextfield } from '@taiga-ui/kit'; 
  ...
  @Component({
    ...
    imports: [
      ReactiveFormsModule, 
      TuiButton, 
      TuiInput,       // Компонент инпута v5
      TuiInputNumber, // Компонент числового инпута v5
      TuiTextfield    // Директива настройки текстового поля v5
    ],
...
```html
  <div class="bg-white p-6 rounded-lg shadow-sm border border-slate-200 max-w-md mx-auto">
    ...
    <form [formGroup]="courseForm" (ngSubmit)="onSubmit()" class="space-y-4">
      ...
      <!--  СИНТАКСИС v5: Директива tuiTextfield настраивает сам компонент tui-input -->
      <tui-input 
      ...
      <!-- Используем специализированный tui-input-number для академических часов -->
      <tui-input-number 
