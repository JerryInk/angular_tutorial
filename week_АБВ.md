...
```html
  ...
  <tui-root class="min-h-screen bg-slate-50 font-sans antialiased">
    
    <header class="bg-white shadow-sm border-b border-slate-200 p-4 mb-6">
      <div class="container mx-auto flex justify-between items-center">
        <h1 class="text-xl font-bold text-slate-800">ИТ-Кафедра | Angular 22</h1>
      </div>
    </header>
...
Открыть course-form.component.ts, импортировать modern Standalone компоненты библиотеки:
```typescript
  import { Component, ChangeDetectionStrategy, inject } from '@angular/core';
  import { ReactiveFormsModule, FormBuilder, Validators } from '@angular/forms';
  import { TuiButton } from '@taiga-ui/core'; // Кнопки core
  import { TuiInputModule, TuiTextfieldControllerModule } from '@taiga-ui/kit'; // Элементы форм kit
  
  @Component({
    selector: 'app-course-form',
    standalone: true,
    // Подключаем модули Taiga UI прямо в Standalone импорты
    imports: [
      ReactiveFormsModule, 
      TuiButton, 
      TuiInputModule, 
  ...
  export class CourseFormComponent {
    ...
    onSubmit(): void {
      ...
      console.log('Данные формы:', this.courseForm.getRawValue());
...
Открыть course-form.component.html. Обратите внимание на то, как лаконично выглядит разметка Taiga UI, управляемая Tailwind-классами отступов (space-y-4, max-w-md):
```html
  <div class="bg-white p-6 rounded-lg shadow-sm border border-slate-200 max-w-md mx-auto">
    <h3 class="text-lg font-bold text-slate-900 mb-4">Добавить новую дисциплину</h3>
    
    <form [formGroup]="courseForm" (ngSubmit)="onSubmit()" class="space-y-4">
      
      <!-- Компонент ввода названия с плавающим текстом-заполнителем -->
      <tui-input formControlName="title" tuiTextfieldSize="m">
        Название дисциплины
        <input tuiTextfield type="text" />
      </tui-input>
  
      <!-- Компонент ввода часов с контроллером размера -->
      <tui-input formControlName="hours" tuiTextfieldSize="m">
        Объем (академические часы)
        <input tuiTextfield type="number" />
      </tui-input>
  
      <!-- Продвинутая кнопка Taiga UI. Стилизуется Tailwind-классом w-full -->
...
## Шаг 1: Интеграция TuiAlertService в Smart-компонент
...
```typescript
  ...
  import { TuiAlertService } from '@taiga-ui/core'; // Импортируем сервис алертов
...
Модифицируем метод добавления новой дисциплины. При успешном ответе от сервера бэкенда мы будем вызывать красивый push-алерт с автоматическим закрытием через 3 секунды.
```typescript
  ...
  handleCreateCourse(newCourseData: Course): void {
    this.apiService.createCourse(newCourseData).subscribe({
      next: (createdItem) => {
        ...
        this.alerts.open('Дисциплина успешно внесена в базу данных бэкенда.', {
          label: 'Успешная синхронизация',
          appearance: 'success', // Зеленый стиль уведомления
          autoClose: 3000        // Закроется само через 3 секунды
  ...
  confirmDelete(id: number | undefined): void {
    ...
    this.dialogs.open<boolean>(
      'Вы уверены, что хотите безвозвратно удалить данную дисциплину из учебного плана семестра?',
      {
        ...
        size: 's',
        data: { button: 'Удалить', cancel: 'Отмена' }
      }
    ).subscribe({
      next: () => {
