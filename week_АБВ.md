...
```typescript
   ...
   import { TuiButton, TuiTextfield } from '@taiga-ui/core'; // Директива TuiTextfield живет в @taiga-ui/core!
   ...
   @Component({
     ...
     imports: [
       ReactiveFormsModule,
       TuiButton,
       TuiTextfield // Импортируем базовую Standalone-директиву текстового поля v5
     ],
...
```html
  <div class="bg-white p-6 rounded-lg shadow-sm border border-slate-200 max-w-md mx-auto font-sans">
    ...
    <form [formGroup]="courseForm" (ngSubmit)="onSubmit()" class="space-y-5">
      ...
      <!-- Поле: Название. Современный синтаксис v5 на базе нативного инпута -->
      <label tuiTextfield class="flex flex-col">
        Название дисциплины
        <input
          tuiTextfield
          type="text"
      ...
      <!-- Поле: Часы. Используем нативный тип number, управляемый директивой Taiga UI -->
      <label tuiTextfield class="flex flex-col">
